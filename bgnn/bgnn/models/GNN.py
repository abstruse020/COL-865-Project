...
class GNN(BaseModel):
    ...
    def fit(self, networkx_graph, X, y, train_mask, val_mask, test_mask, num_epochs,
        ...
        if self.gbdt_predictions is not None:
            ...
            pred_dim = len(self.gbdt_predictions.shape)
            if pred_dim > 1:
                pred_dim = len(self.gbdt_predictions[0])
                extra_cols = [f"predict_{i}" for i in range(pred_dim)]
                X[extra_cols] = self.gbdt_predictions
            else:
                extra_cols = ['predict']
